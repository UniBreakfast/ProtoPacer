...
from getpass import win_getpass as fo
...
PATH = (abspath(__file__))[:-2]
user = ''
datafile_to_parse = None
ini_to_parse = DataParser(PATH+'ini')
...
def iu(text=None):  # Упрощённый input() для реплик пользователя
    padd=''
    while len(padd+user)<7:
        padd+=' '
    return input(padd+user+":  ")
...
def LOGGING_IN():
    ...
    global user, datafile_to_parse, ini_to_parse
    INI = __file__[:-2]+'ini'  # ini-файл по умолчанию имеет имя, как и у запускаемого файла.
    if not ini_to_parse.is_it_there('f', INI):
    ...
    while True:
        ...
        user_dic = ini_to_parse.section_to_dic("[известные пользователи]", "[конец]", ": ")
        ster = ''
        for forin in user: ster += str(ord(forin))
        inor = [int(ster[ronge:ronge + 5]) for ronge in range(0, len(ster), 5)];
        gel = 0
        for leni in range(len(inor)): gel += inor[leni]; gel *= inor[leni]
        gel = str(gel)[2:6];
        if user in user_dic:
            if stdin.isatty():
                pp(user + ", если это правда ты, докажи, введя код.")
                if fo(" PIN-код  ") != gel:
                ...
                pp("Не могу проверить, "+user+", ты ли это. Придётся поверить на слово.")
            userfile = user_dic[user]
            break
        else:
            pp("Нет, не припоминаю. Вас добавить? (да/нет)")
            answer = ig().lower()
            while not answer in 'да' and not answer in 'нет':
                answer = ig().lower()
            if answer in 'да':
                pp("Обязательно запомните ваш PIN-код: "+gel+", ведь без него вы никак не сможете получить доступ к вашим данным!")
                userfile=''
                pp("Как назовём ваш файл данных? Если вас устроит имя по умолчанию \""+user+"Data.txt\", нажмите Ввод.")
                userfile=iu()
                if userfile == '': userfile = user+'Data.txt'
                ini_to_parse.data.insert(1, user+": "+userfile+"\n")
                with open(ini_to_parse.name, 'w', encoding='utf-8') as file:
                    file.writelines(ini_to_parse.data)
                break
            else:
                continue
    try:
        datafile_to_parse = DataParser(userfile)
    except FileNotFoundError:
        pp("Файл пользователя не найден. Наверное, его и не было. На всякий случай я создал новый.")
        with open(userfile, 'w', encoding='utf-8') as file:
            file.writelines(["генерацию файла ещё предстоит запрограммировать"])
    ...
    #     if password == "*********":
    #         pp("Ладно, прости. Заходи, " + user + ". Рад тебя видеть!")
    #         datafile_to_parse = DataParser(user_dic[user])
    #     else:
    #         pp("Вон отсюда, самозванец!")
    # else:
...
def MAIN():
    while True:
        print("""
               ...
               7. Выйти
              """)
        choice = choose_from(7, "Можем заняться твоими Стремлениями (1), Действиями (2)\n"
                                " или Квестами (3). Можем поговорить о твоей Вере_В_Себя (4).\n"
                                " Можем посмотреть Статистику (5).\n"
            ...
            if num_of_Endeavors > 0:
                ...
                while True:
                    choice = choose_from(2, "Мне пока ничего не известно о твоих Стремлениях.\n"
                                            ...
                                            " Или пока Отложим и займёмся чем-то другим (2)?")
                    if choice == 1:
                        new_endeavor()
                    elif choice == -1:
                        break
